    ...
    >
      ...
      >
        <div class="border-2 border-white shadow-2xl p-4 rounded-md">
          <form action="/Manager-PT/addPlayer_exe.php" method="post" enctype="multipart/form-data" class="flex flex-col gap-4">
            <div class="flex items-center gap-2 text-white">
              <label for="name">Nome:</label>
              <input type="text" id="name" name="name" class="bg-transparent border-b-1 focus:outline-none focus:bg-[#017d38] border-white p-2 transition-colors duration-300" autocomplete="off" required>
            </div>

            <div class="flex items-center gap-2 text-white">
              <label for="dateOfBirth">Data de Nascimento:</label>
              <input type="date" id="dateOfBirth" name="dateOfBirth" class="bg-transparent border-b-1 focus:outline-none focus:bg-[#017d38] border-white p-2 transition-colors duration-300" max="<?php echo date('Y-m-d'); ?>" required>
            </div>
            ...
            <div class="flex items-center gap-2 text-white">
              ...
              <div class="relative flex items-center justify-center gap-2 w-full h-60 hover:bg-[#017d38] border-1 border-white rounded-md p-2 transition-colors duration-300">
                <label for="imagem" class="flex items-center justify-center w-full h-full cursor-pointer">
                  <p>Selecionar Imagem</p>
                  <input type="file" id="imagem" name="image" accept="image/*" class="hidden" required>
                </label>

                <img id="imagem-preview" src="" alt="Pré-visualização da Imagem" class="w-full h-full object-cover hidden">

                <div id="remover-imagem" class="hidden absolute top-2 right-2 cursor-pointer bg-[#cd0101] hover:bg-[#ad0101] text-white rounded-full px-2 transition-colors duration-300">X</div>
              </div>
            </div> 
